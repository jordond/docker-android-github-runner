...
yes | ${ANDROID_SDK_ROOT}/cmdline-tools/bin/sdkmanager --sdk_root=${ANDROID_HOME} --licenses >/dev/null
${ANDROID_SDK_ROOT}/cmdline-tools/bin/sdkmanager --sdk_root=${ANDROID_HOME} --list 1>/dev/null
if [ $? -eq 0 ]
then
...
# Install the following SDKs and build cmdline-tools, passing in "y" to accept licenses.
echo "y" | ${ANDROID_SDK_ROOT}/cmdline-tools/bin/sdkmanager --sdk_root=${ANDROID_HOME} \
    "platform-tools" \
